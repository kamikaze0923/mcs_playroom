...
from algorithms.point_goal_navigation.ppo.utils import batch_obs
...
if __name__ == '__main__':
    ...
    while True:
        ...
        mask = torch.tensor([[1]])
        hidden_states = torch.zeros(size=(1,1,512))
        while not done:
            batch = batch_obs(obs)
            action, hidden_states = nav.act(batch, hidden_states, mask)
