    ...
    if object_state.velocity[0] < 0:
        for occluder_name, occluder_state in all_occluder_dicts.items():
            ...
            if occuluder_dis >= 0 and occuluder_dis < closest_occuluder_dis:
                ...
                min_pixel_dis = object_state.pixels_on_frame['x_min'] - occluder_state.pixels_on_frame['x_max']
        ...
        for occluder_name, occluder_state in all_occluder_dicts.items():
            ...
            if occuluder_dis >= 0 and occuluder_dis < closest_occuluder_dis:
                closest_occuluder_dis = occuluder_dis
                min_pixel_dis = occluder_state.pixels_on_frame['x_min'] - object_state.pixels_on_frame['x_max']
                which_occluder = occluder_name
    else:
        ...
        for occluder_name, occluder_state in all_occluder_dicts.items():
            ...
            if occuluder_dis < closest_occuluder_dis:
                closest_occuluder_dis = occuluder_dis
                min_pixel_dis = occluder_state.pixels_on_frame['y_min'] - object_state.pixels_on_frame['y_max']
                which_occluder = occluder_name
...
def get_running_out_occluder_info(all_occluder_dicts, appear_state, estimate_state):
    ...
    if estimate_state.velocity[0] < 0:
        for occluder_name, occluder_state in all_occluder_dicts.items():
            ...
            if occuluder_dis >= 0 and occuluder_dis < closest_occuluder_dis:
                closest_occuluder_dis = occuluder_dis
                min_pixel_dis = occluder_state.pixels_on_frame['x_min'] - appear_state.pixels_on_frame['x_max']
                which_occluder = occluder_name
        ...
        for occluder_name, occluder_state in all_occluder_dicts.items():
            ...
            if occuluder_dis >= 0 and occuluder_dis < closest_occuluder_dis:
                closest_occuluder_dis = occuluder_dis
                min_pixel_dis = appear_state.pixels_on_frame['x_min'] - occluder_state.pixels_on_frame['x_max']
                which_occluder = occluder_name
    else:
        ...
        for occluder_name, occluder_state in all_occluder_dicts.items():
            ...
            if occuluder_dis < closest_occuluder_dis:
                closest_occuluder_dis = occuluder_dis
                min_pixel_dis = appear_state.pixels_on_frame['y_min'] - occluder_state.pixels_on_frame['y_max']
                which_occluder = occluder_name
    return min_pixel_dis, which_occluder
...
class OccluderState:
    def __init__(self, object_info, depth_frame, object_frame, agent_position):
        ...
        self.depth, self.pixels_on_frame = get_occluder_frame_info(object_info, depth_frame, object_frame)
        # print(min(self.pixels_on_frame), max(self.pixels_on_frame))
