...
class FieldOfView:
    ...
    def getFoVPolygon(self, maxLen=25, eps=0.000001):
        poly_X = []
        poly_Y = []
        poly_angle = []
        ...
        # cast on HFOV lines
        for i in np.arange(lAngle, rAngle, 5 / 180 * math.pi):
            v = Geometry.Point(p1.x + maxLen * math.sin(i), p1.y + maxLen * math.cos(i))
        ...
        for obs in self.obstacle:
            ...
            for x, y in obs.exterior.coords:
                v = Geometry.Point(x, y)

                if self.isLeftOfLine(p1, p2R, v) and not self.isLeftOfLine(p1, p2L, v) or self.HVoF >= 2 * math.pi:
                    # cast at point
                    # plt.plot(v.x,v.y,"or")
        ...
        poly_X = list(np.array(poly_X)[indx])
        poly_Y = list(np.array(poly_Y)[indx])

        if self.HVoF < 2 * math.pi:
            poly_X = [p1.x] + poly_X + [p1.x]
            poly_Y = [p1.y] + poly_Y + [p1.y]

        return ObstaclePolygon(zip(poly_X, poly_Y))
    ...
    def castRay(self, angle, maxLen, clr="-g"):
        ...
        minY = p2.y
        for obs in self.obstacle:
            for i in range(len(obs.exterior.coords) - 1):
                o1 = Geometry.Point(obs.exterior.coords[i][0], obs.exterior.coords[i][1])
                o2 = Geometry.Point(obs.exterior.coords[i + 1][0], obs.exterior.coords[i + 1][1])
